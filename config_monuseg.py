...
class Config(object):
    ...
    def __init__(self):
        ...
        input_shape = [256, 256] # patch shape used as input to network - central crop performed after augmentation
        out_shape = [164, 164] # patch shape at output of network

        if model_mode == "original":
            if input_shape != [270,270] or out_shape != [80,80]:
                raise Exception("If using `original` mode, input shape must be [270,270] and output shape must be [80,80]")
        if model_mode == "fast":
            if input_shape != [256,256] or out_shape != [164,164]:
        ...
        self.shape_info = {
            "train": {"input_shape": input_shape, "mask_shape": out_shape,},
            "valid": {"input_shape": input_shape, "mask_shape": out_shape,},
        }
